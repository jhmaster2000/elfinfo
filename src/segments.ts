...
/*export async function readProgramHeaderEntries(
    fh: Reader, ph_off: number | bigint, ph_entsize: number, ph_num: number, bits: number, bigEndian: boolean): Promise<ELF.Segment[]> {

    if (ph_num === 0) return [];

    const result = new Array<ELF.Segment>(ph_num);
